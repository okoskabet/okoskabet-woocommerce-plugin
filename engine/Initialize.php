...
namespace okoskabet_woocommerce_plugin\Engine;

use okoskabet_woocommerce_plugin\Engine;

/**
 * okoskabet_woocommerce_plugin Initializer
 */
class Initialize {
	...
	private function load_classes() {
		$this->classes = \apply_filters( 'okoskabet_woocommerce_plugin_classes_to_execute', $this->classes );

		foreach ( $this->classes as $class ) {
			try {
				$this->initialize_plugin_class( $class );
			} catch ( \Throwable $err ) {
				\do_action( 'okoskabet_woocommerce_plugin_initialize_failed', $err );
	...
	private function initialize_plugin_class( $classtovalidate ) {
		...
		\add_filter(
			'okoskabet_woocommerce_plugin_instance_' . $classtovalidate,
			function() use ( $temp ) {
				return $temp;
			}
		);
	...
	private function get_classes( string $namespacetofind ) {
		...
		$namespacetofind = 'okoskabet_woocommerce_plugin\\' . $namespacetofind;

		// In case composer has autoload optimized
		if ( isset( $classmap[ 'okoskabet_woocommerce_plugin\\Engine\\Initialize' ] ) ) {
		...
		if ( isset( $prefix[ $namespacetofind ] ) ) {
			...
			if ( !WP_DEBUG ) {
				\wp_die( \esc_html__( 'okoskabet-woocommerce-plugin is on production environment with missing `composer dumpautoload -o` that will improve the performance on autoloading itself.', O_TEXTDOMAIN ) );
			}
